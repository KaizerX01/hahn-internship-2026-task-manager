...
import { Plus, Search, MoreVertical, Trash2, Edit, CheckCircle2, Circle, Clock, Loader2, ArrowLeft, Calendar, X } from 'lucide-react';
...
const TaskCard: React.FC<TaskCardProps> = ({ task, onEdit, onDelete, onToggle }) => {
  ...
  return (
    <div 
      className={`group relative bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl rounded-2xl p-6 border ${getBorderColor()} hover:border-violet-500/50 transition-all duration-500 hover:shadow-2xl hover:shadow-violet-500/20 overflow-hidden`}
    >
...
const TasksPage: React.FC = () => {
  ...
  const { tasks, loading, error, createTask, updateTask, deleteTask, toggleCompletion, updateFilters } = useTasks(projectIdNum);
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-12">
        <div className="mb-12">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              <button
                onClick={handleBack}
                className="p-3 bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-xl hover:border-violet-500/50 transition-all duration-300"
              >
        ...
        {tasks.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold text-slate-400 mb-2">No tasks yet</h3>
